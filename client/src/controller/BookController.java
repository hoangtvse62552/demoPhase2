...
public class BookController
{
    ...
    public boolean updateBook(Book book)
    {
        ...
        if (con != null)
        {
            try
            {
                String sql = "UPDATE DB2ADMIN.BOOKS SET NAME= ?, DESCRIPTION= ?, UPDATE_TIME= ?, PUBLISHER_ID= ? WHERE BOOK_ID= ?";
                stm = con.prepareStatement(sql);
...
//                System.out.println("result of update book: " + rs);
    ...
    public boolean addBook(Book book)
    {
        ...
        if (con != null)
        {
            try
            {
                String sql = "SELECT BOOK_ID FROM FINAL TABLE ( INSERT INTO DB2ADMIN.BOOKS (NAME, DESCRIPTION, CREATE_TIME, UPDATE_TIME, PUBLISHER_ID) VALUES(?, ?, ?, ?, ?))";
//                String sql = "INSERT INTO BOOKS (NAME, DESCRIPTION, CREATE_TIME, UPDATE_TIME, PUBLISHER_ID) VALUES( ?, ?, ?, ?, ?)";
    ...
    public boolean deleteBook(int id)
    {
        ...
        if (con != null)
        {
            try
            {
                String sql = "DELETE FROM DB2ADMIN.BOOKS WHERE BOOK_ID= ?";
                stm = con.prepareStatement(sql);
                stm.setInt(1, id);
                int rs = stm.executeUpdate();
                if (rs != -1)
